use littlefs2::{fs::DirEntry, io::Error, object_safe::DynFilesystem, path};
use trussed::types::{Path, PathBuf};

fn ignore_does_not_exists(error: Error) -> Result<(), Error> {
    if matches!(error, Error::NoSuchEntry) {
        return Ok(());
    }
    Err(error)
}
...
fn migrate_rp_dir(
    ...
    fs.read_dir_and_then(rp_path, &mut |rp_dir| {
        for file in rp_dir.skip(2) {
            ...
            fs.rename(file.path(), &rk_dir.join(&PathBuf::from(buf.as_slice())))?;
        }
        Ok(())
    })?;
